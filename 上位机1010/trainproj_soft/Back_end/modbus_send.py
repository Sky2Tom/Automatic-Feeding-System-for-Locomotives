...
class Serial_Qthread_function(QObject):
    ...
    def slot_readyRead(self):
        ...
        while(self.serial_master.bytesAvailable):
            buf = self.serial_master.readAll() 
            self.recv_buffer.extend(bytes(buf.data())) # 拼接到缓冲区
            
        if len(self.recv_buffer) >= 5: # 简易条件，实际可更严格：最少 5 字节，且 CRC 正确等
            frame = bytes(self.recv_buffer) # 复制一份
            print(f"收到数据: {frame.hex()}")
            self.signal_readyRead.emit({'buf': frame.hex()}) # 保持原有信号，避免破坏旧逻辑
            self.signal_frameReceived.emit(frame) # 新增：发出原始 bytes
