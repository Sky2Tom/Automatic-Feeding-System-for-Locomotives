...
if __name__ == "__main__":
    ...
    test_frame = [
        ...
        0x03,       # 功能码 (读取保持寄存器)
        0x00, 0x08, # 起始地址 (0008)
        0x00, 0x05, # 寄存器数量 (0005)
        0x04, 0x0B  # CRC16 校验码 (预先计算好的)
    ]
    ...
    # 4. 打印解析结果
    print("\n帧解析结果:")
    print(f"状态码: 0x{uart_info.Mdbs_state:02x}")
    if uart_info.Mdbs_state & Frame_OK:
        print("状态: 帧有效 (Frame_OK)")
        print(f"从站地址: 0x{uart_info.RxAddr:02x}")
        print(f"功能码: 0x{uart_info.RxFuncID:02x}")
        print(f"数据: {uart_info.RxData[:uart_info.RxTimes]}")
    else:
        print("状态: 帧无效")
